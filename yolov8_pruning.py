...
def prune(args):
    ...
    model = YOLO("yolov8-onechannel-relu.yaml").load(args.model)
    #model = YOLO("yolov8.yaml").load(args.model)
    ...
    example_inputs = torch.randn(1, 1, pruning_cfg["imgsz"], pruning_cfg["imgsz"]).to(model.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', default=1, type=int, help='number epochs per iteration')
