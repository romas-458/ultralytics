...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, eps=1e-7):
    ...
    if CIoU or DIoU or GIoU or SIoU:
        ...
        if SIoU:  # Scaled IoU (SIoU)
            px = (b1_x1 + b1_x2) / 2 - (b2_x1 + b2_x2) / 2  # X-axis center difference
            py = (b1_y1 + b1_y2) / 2 - (b2_y1 + b2_y2) / 2  # Y-axis center difference

            sigma = torch.exp(-((px ** 2 + py ** 2) / (cw ** 2 + ch ** 2 + eps)))  # Scaling factor

            angle = torch.atan(h2 / w2) - torch.atan(h1 / w1)  # Angle difference
            angle_loss = (4 / math.pi ** 2) * angle.pow(2)

            return iou * sigma - angle_loss  # SIoU
